...
| `xai-grok`       | `--always-approve`                    | Read-only adds `--disallowed-tools search_replace,run_terminal_command`; `write` stays allowed. No `--sandbox`, no `--add-dir` — extra roots are a named over-grant |

...
- [project_plan_checks_conformance_firstrun_review.md](project_plan_checks_conformance_firstrun_review.md) — plan-check leaf / conformance suite / demo --script review: literal NUL bytes in a test file (git diff renders it binary) + demo --script's checkCli skip scoped to implement only while the seeded sandbox invites Plan/Refine too

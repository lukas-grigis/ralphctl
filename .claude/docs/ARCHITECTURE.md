...
| `HeadlessAiProvider`                                  | `integration/ai/providers/_engine/` | `claude` / `copilot` / `codex` / `opencode` / `grok` adapters under `providers/<tool>/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
- **`attachAbortKill`** (`providers/_engine/abort-kill.ts`) — wires a caller `AbortSignal` to a
  ...
  by all five interactive adapters so a TUI cancel consistently terminates the AI child regardless of
  provider.
- **`BoundedTail` / `createBoundedTail`** (`providers/_engine/bounded-tail.ts`) — rolling tail
  ...
  (`createCappedLineFeed`, `providers/_engine/line-feed.ts`) shared by all five stream parsers
  (Claude / Copilot / Codex / OpenCode / Grok) is also bounded (`STDOUT_LINE_PARSE_CAP` = 512 KiB) — a
...
| `operator` | `integration/ai/skills/operator/` | Global, per-provider drop-ins under `<appRoot>/skills/<claude,copilot,codex,opencode,grok>/<name>/SKILL.md`                                          |
...
Five per-provider renderers (`_engine/render-{claude,copilot,codex,opencode,grok}-agent.ts`) turn one canonical
...
| Grok     | `renderGrokAgent`     | `.grok/agents/ralphctl-<name>.md`         | YAML frontmatter (`name`, `description`, `model?`) + Markdown body; Claude-only keys (`permissionMode`, `effort`, `tools`) are not emitted                                                     |
...
`AgentDefinitionAdapter` implementation shared by all five providers — `claude/adapter.ts`,
`copilot/adapter.ts`, `codex/adapter.ts`, `opencode/adapter.ts`, `grok/adapter.ts` each just supply `parentDir` + `renderer` +
convention text,
...
- **`Settings`** — declared by `SettingsSchema` in `domain/entity/settings.ts`. Top-level fields:
  ...
  `'claude-code' | 'github-copilot' | 'openai-codex' | 'opencode' | 'xai-grok'`. Optional `ai.skills` is the durable

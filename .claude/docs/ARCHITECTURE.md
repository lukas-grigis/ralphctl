...
└── ticket/          add / edit / remove / approve / assign-repositories
```
...
│                                   mark-in-progress → execute-task (Retry on rate-limit) →
...
| Plan      | `load-sprint → assert-draft → assert-all-approved → link-skills → run-plan → reorder-dependencies → unlink-skills`                                                                                                                                        |
| Ideate    | `load-sprint → assert-draft → run-ideation → reorder-dependencies`                                                                                                                                                                                        |
| Execute   | `load-sprint → assert-active → load-tasks → check-scripts-sprint-start → link-skills → execute-tasks → unlink-skills` (per-task: `branch-preflight → mark-in-progress → execute-task → post-task-check → recover-dirty-tree → evaluate-task → mark-done`) |
...
// Mutators relevant to Run B: `Sprint.rename(name)`, `Sprint.clearBranch()`,
// `Sprint.recordPullRequestUrl(url)`. Repository: `markOnboarded(now)`,
// `clearOnboarded()`, `withSetupScript(script)`. Task: `update(input)`,
// `markBlocked(reason)`, `unblock()`.
```
...
class Ticket {
  ...
  readonly affectedRepositories: readonly AbsolutePath[] | undefined; // set by sprint plan

...
| `ModelAvailabilityProbe`                              | `integration/ai/providers/_engine/` | per-provider probe, total over `AiProvider` (`buildModelAvailabilityProbes(logger)` in `bootstrap/wire.ts`) — narrows the static catalog to what the account can run; fails open to the full catalog on any error. `opencode`'s probe (`opencode models`) is the one exception permitted to return ids OUTSIDE the static catalog, since the aggregator's reachable set depends on which upstream providers the operator authenticated — and the one whose fallback is lossy (free tier only), so its fail-opens report a `ModelProbeDegradation` that `wire()` logs at warn |

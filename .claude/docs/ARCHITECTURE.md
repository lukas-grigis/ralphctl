...
| `HeadlessAiProvider`                                  | `integration/ai/providers/_engine/` | `claude` / `copilot` / `codex` / `opencode` adapters under `providers/<tool>/`                                                                                                                                                                                                                                                                                                                                              |
| `InteractiveAiProvider`                               | `integration/ai/providers/_engine/` | same per-tool adapters (interactive entrypoint)                                                                                                                                                                                                                                                                                                                                                                             |
| `ModelAvailabilityProbe`                              | `integration/ai/providers/_engine/` | per-provider probe, total over `AiProvider` (`MODEL_AVAILABILITY_PROBES` in `bootstrap/wire.ts`) — narrows the static catalog to what the account can run; fails open to the full catalog on any error. `opencode`'s probe (`opencode models`) is the one exception permitted to return ids OUTSIDE the static catalog, since the aggregator's reachable set depends on which upstream providers the operator authenticated |
...
| `AgentDefinitionAdapter` + `AgentDefinitionSource`    | `integration/ai/agents/_engine/`    | per-provider adapter (`claude` / `copilot` / `codex` / `opencode` under `agents/<tool>/`, all backed by `createFilesystemAgentDefinitionAdapter`) + bundled / operator sources composed by `composeAgentDefinitionSources`; `parseAgentDefinition` extracts an `AgentDefinition` from frontmatter Markdown; `checkAgentDefinitionQuality` flags vague bodies — see § Agent definitions subsystem                            |
...
- **`attachAbortKill`** (`providers/_engine/abort-kill.ts`) — wires a caller `AbortSignal` to a
  ...
  by all four interactive adapters so a TUI cancel consistently terminates the AI child regardless of
  provider.
- **`BoundedTail` / `createBoundedTail`** (`providers/_engine/bounded-tail.ts`) — rolling tail
  ...
  forensic body mirrors (`FORENSIC_BODY_TAIL_CAP` = 256 KiB). The NDJSON line-parse accumulator
  (`createCappedLineFeed`, `providers/_engine/line-feed.ts`) shared by all four stream parsers
  (Claude / Copilot / Codex / OpenCode) is also bounded (`STDOUT_LINE_PARSE_CAP` = 512 KiB) — a
...
| `operator` | `integration/ai/skills/operator/` | Global, per-provider drop-ins under `<appRoot>/skills/<claude,copilot,codex,opencode>/<name>/SKILL.md`                                               |
...
Four per-provider renderers (`_engine/render-{claude,copilot,codex,opencode}-agent.ts`) turn one canonical
...
| OpenCode | `renderOpencodeAgent` | `.opencode/agents/ralphctl-<name>.md`     | YAML frontmatter (`description`, `mode: subagent`, `model?`) + Markdown body; no `name` key (OpenCode derives it from the filename); `effort` dropped (spelled `--variant` at invocation time) |
...
- **`Settings`** — declared by `SettingsSchema` in `domain/entity/settings.ts`. Top-level fields:
  ...
  `'claude-code' | 'github-copilot' | 'openai-codex' | 'opencode'`. Optional `ai.skills` is the durable
  ...
  (`0` disables — default; `2`–`4` grants one best-of-N candidate-sampling attempt at the top of the
  plateau escalation ladder) — see `PERFORMANCE.md` § Escalation on plateau.
...
| `RefinedTicketSignal`                                    | `refine` flow projects the AI-authored requirements `body` onto the `PendingTicket` entity via `refineTicketUseCase` (approval gated by an optional reviewer callback); no sidecar                                                                                                                                                                                                                                                                            |
...
- **Real-provider e2e** — every Claude / Copilot / Codex / OpenCode provider test uses a fake `spawn`. Vendor
  JSON-shape drift will surface here first. Same gap as v0.6.x; deferred.

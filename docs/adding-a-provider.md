...
| `additionalRoots`                     | `InvalidStateError` — never silently run with only `cwd`. Named over-grant is the documented exception when the CLI has no `--add-dir` **and** sandbox is off (OpenCode `--auto`, Grok) — extra roots then work unrestricted, so claiming a subset would be a lie. |

...
export interface SessionDescriptor<TCtx = unknown> {
  ...
  /**
   * Optional uniqueness key. When non-empty, `start()` checks for an
   * existing non-terminal session with the same key and returns its id
   * instead of creating a new one. Used to make workflows like onboard
   * single-instance per project.
   */
  readonly dedupeKey?: string;
  /**
   * Whether this session can be backgrounded (UI detached while the
   * runner keeps going). Defaults to true. Foreground-only flows like
   * onboard (which require the user's interactive prompts) set this to
   * false; the execute view rejects the `D` keypress for them.
   */
...
export interface SessionManagerStartOptions<TCtx> {
  ...
  /**
   * When set, an existing non-terminal session with the same `dedupeKey`
   * blocks the new start — `start()` returns the existing session's id
   * instead of registering a duplicate. Use for single-instance flows
   * (e.g. onboard per project: `dedupeKey: 'onboard:${projectName}'`).
   */
  readonly dedupeKey?: string;
  /**
   * False for foreground-only flows (interactive prompts mid-chain).
   * Defaults to true (chains can be backgrounded). When false, the
   * execute view's `D` (background) key is a no-op.
   */
  readonly detachable?: boolean;
}

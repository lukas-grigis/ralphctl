...
import { makeSprint, makeTask } from '../../_test-fakes/fixtures.ts';
...
describe('loadTasksLeaf', () => {
  it('reads the task list for a sprint and writes it onto the context', async () => {
    ...
    if (!result.ok) return;
    expect(result.value.ctx.tasks?.map((t) => t.name)).toEqual(['one', 'two']);
  });
  ...
  it('returns an empty list (not a domain error) for sprints without tasks', async () => {
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.value.ctx.tasks).toEqual([]);
  });
});

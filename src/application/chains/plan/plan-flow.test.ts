...
import { abs, makeApprovedTicket, makeSprint, makeTicket } from '../../_test-fakes/fixtures.ts';
...
describe('createPlanFlow', () => {
  it('runs load-sprint → assert-draft → assert-all-tickets-approved → load-existing-tasks → plan-tasks → reorder-tasks → save-tasks', async () => {
  ...
  it('fails at assert-all-tickets-approved when a ticket is still pending', async () => {
    ...
    expect(stepNames.slice(0, 3)).toEqual(['load-sprint', 'assert-draft', 'assert-all-tickets-approved']);
    expect(result.error.trace[2]?.status).toBe('failed');
    // Steps after the failure are skipped.
    expect(stepNames.slice(3)).toEqual(['load-existing-tasks', 'plan-tasks', 'reorder-tasks', 'save-tasks']);

...
describe('FanOutLogger', () => {
  ...
  it('fans success() to every sink with level: "success"', () => {
    const a = new FakeLoggerPort();
    const b = new FakeLoggerPort();
    const fan = new FanOutLogger([a, b]);
    fan.success('task done');
    expect(a.entries).toHaveLength(1);
    expect(b.entries).toHaveLength(1);
    expect(a.entries[0]?.level).toBe('success');
    expect(b.entries[0]?.level).toBe('success');
    expect(a.entries[0]?.message).toBe('task done');
  });
  ...
  it('child returns a FanOutLogger that bound context flows through', () => {
    ...
    child.info('hi');
    expect(a.entries[0]?.context).toStrictEqual({ scope: 'unit' });
  });

...
export interface DoctorCheckResult {
  ...
  /** Short one-line summary (e.g. `"6 repos not onboarded"`). */
  readonly message?: string;
  /**
   * Per-item details rendered as indented bullets under the row. Use this
   * when the check has more than ~2 items to surface — keeps the inline
   * message scannable while still exposing the full list.
   */
...
export async function runDoctor(deps: SharedDeps): Promise<DoctorReport> {
  ...
  const checks = await Promise.all([
    ...
    onboardingStatusCheck({ projectRepo: deps.projectRepo, configStore: deps.configStore }),

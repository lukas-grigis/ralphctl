 ...
 *
 * Animates at 80ms per frame (roughly 12fps). Uses `setInterval` rather than
 * requestAnimationFrame because Ink reconciles on a timer anyway.
 *
...
export function Spinner({ label, color = inkColors.warning, intervalMs = 80 }: SpinnerProps): React.JSX.Element | null {
  ...
  // UI contract: while a view is idle waiting on a prompt, the prompt owns
  // the visual — don't show a spinner.

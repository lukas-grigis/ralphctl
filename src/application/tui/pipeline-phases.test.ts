...
describe('computePipelineSnapshot', () => {
  ...
  describe('Refine phase', () => {
    it('suggests Create Sprint when no sprint exists and projects exist', () => {
      ...
      expect(refine?.action).toEqual({ kind: 'route', viewId: 'sprint-create', label: 'Create Sprint' });
    ...
    it('suggests Add Project when no sprint exists and no projects exist', () => {
      ...
      expect(refine?.action).toEqual({ kind: 'route', viewId: 'project-add', label: 'Add Project' });
      expect(snap.currentPhaseId).toBe('refine');
      expect(snap.nextStep).toEqual(refine?.action);
    });
    ...
    it('suggests Add Ticket when a draft sprint has no tickets', () => {
      ...
      expect(snap.phases[0]?.action).toEqual({ kind: 'route', viewId: 'ticket-add', label: 'Add Ticket' });
    });
  ...
  describe('Plan phase', () => {
    ...
    it('becomes active with a Plan Tasks action once all requirements are approved and no tasks exist', () => {
      ...
      expect(snap.phases[1]?.action).toEqual({ kind: 'launchChain', flow: 'plan', label: 'Plan Tasks' });
  ...
  describe('Close phase', () => {
    it('offers Close Sprint on an active sprint with all tasks done', () => {
      ...
      expect(snap.phases[3]?.action).toEqual({ kind: 'route', viewId: 'sprint-close', label: 'Close Sprint' });
    ...
    it('is done on a closed sprint', () => {
      ...
      expect(snap.nextStep).toEqual({ kind: 'route', viewId: 'sprint-create', label: 'Start a new sprint' });
    });
    ...
    it('offers Create PR / MR when active+done and sprint has branch but no PR yet', () => {
      ...
      expect(snap.phases[3]?.status).toBe('active');
      expect(snap.phases[3]?.action).toEqual({
        kind: 'launchChain',
        flow: 'create-pr',
        label: 'Create PR / MR',
      });

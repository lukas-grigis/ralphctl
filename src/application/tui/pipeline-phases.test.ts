...
describe('computePipelineSnapshot', () => {
  it('returns four phases in fixed order: refine, plan, execute, close', () => {
    ...
    expect(snap.phases.map((p) => p.id)).toStrictEqual(['refine', 'plan', 'execute', 'close']);
  });

  describe('Refine phase', () => {
    it('suggests Create Sprint when no sprint exists and projects exist', () => {
      ...
      expect(refine?.action).toStrictEqual({ kind: 'route', viewId: 'sprint-create', label: 'Create Sprint' });
      expect(snap.currentPhaseId).toBe('refine');
      expect(snap.nextStep).toStrictEqual(refine?.action);
    });

    it('suggests Add Project when no sprint exists and no projects exist', () => {
      const snap = computePipelineSnapshot(baseCtx({ currentSprintId: null, hasProjects: false }));
      const refine = snap.phases[0];
      expect(refine?.status).toBe('pending');
      expect(refine?.detail).toBe('add a project first');
      expect(refine?.action).toStrictEqual({ kind: 'route', viewId: 'project-add', label: 'Add Project' });
      expect(snap.currentPhaseId).toBe('refine');
      expect(snap.nextStep).toStrictEqual(refine?.action);
    });
    ...
    it('suggests Add Ticket when a draft sprint has no tickets', () => {
      ...
      expect(snap.phases[0]?.action).toStrictEqual({ kind: 'route', viewId: 'ticket-add', label: 'Add Ticket' });
    });
    ...
    it('marks itself active when some tickets still need refinement', () => {
      ...
      expect(snap.phases[0]?.action).toMatchObject({ kind: 'launchChain', flow: 'refine' });
    });
  ...
  describe('Plan phase', () => {
    ...
    it('becomes active with a Plan Tasks action once all requirements are approved and no tasks exist', () => {
      ...
      expect(snap.phases[1]?.action).toStrictEqual({ kind: 'launchChain', flow: 'plan', label: 'Plan Tasks' });
  ...
  describe('Execute phase', () => {
    ...
    it('offers Continue Work on an active sprint with remaining tasks', () => {
      ...
      expect(snap.phases[2]?.action).toMatchObject({ kind: 'launchChain', flow: 'execute' });
    ...
    it('returns no action on a closed sprint with all tasks done', () => {
      const snap = computePipelineSnapshot(
        baseCtx({
          currentSprintId: SPRINT_ID,
    ...
    it('returns no action on a closed sprint even if tasks are still in progress', () => {
      // Belt-and-suspenders: a closed sprint with tasks left in_progress
      // (e.g. the pre-fix Bug 1 state) must NOT dangle a "Continue Work"
      // CTA on Home — the chain's assert-active leaf would reject it
      // anyway. The action is unconditionally null on `closed`.
      const snap = computePipelineSnapshot(
        baseCtx({
          currentSprintId: SPRINT_ID,
          currentSprintStatus: 'closed',
  ...
  describe('Close phase', () => {
    it('offers Close Sprint on an active sprint with all tasks done', () => {
      ...
      expect(snap.phases[3]?.action).toStrictEqual({ kind: 'route', viewId: 'sprint-close', label: 'Close Sprint' });
      expect(snap.currentPhaseId).toBe('close');
      expect(snap.nextStep).toStrictEqual(snap.phases[3]?.action);
    });
    ...
    it('is done on a closed sprint', () => {
      ...
      expect(snap.nextStep).toStrictEqual({ kind: 'route', viewId: 'sprint-create', label: 'Start a new sprint' });
    });
    ...
    it('offers Create PR / MR when active+done and sprint has branch but no PR yet', () => {
      const snap = computePipelineSnapshot(
        baseCtx({
          currentSprintId: SPRINT_ID,
          currentSprintStatus: 'active',
          ...
          tasksDone: 2,
          plannedTicketCount: 1,
          currentSprintHasBranch: true,
          currentSprintHasPullRequest: false,
        })
      );
      expect(snap.phases[3]?.status).toBe('active');
      expect(snap.phases[3]?.action).toStrictEqual({
        kind: 'launchChain',
        flow: 'create-pr',
        label: 'Create PR / MR',
      });
    ...
    it('falls back to Close Sprint once a PR has been recorded', () => {
      const snap = computePipelineSnapshot(
        baseCtx({
          currentSprintId: SPRINT_ID,
          currentSprintStatus: 'active',
          ...
          plannedTicketCount: 1,
          currentSprintHasBranch: true,
          currentSprintHasPullRequest: true,
        })
      );
      expect(snap.phases[3]?.action).toStrictEqual({
        kind: 'route',
  ...
  describe('nextStep selection', () => {
    it('points to the first non-done phase', () => {
      ...
      expect(snap.currentPhaseId).toBe('plan');
      expect(snap.nextStep).toMatchObject({ kind: 'launchChain', flow: 'plan' });
    });

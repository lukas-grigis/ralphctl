...
function computeExecute(ctx: MenuContext): Phase {
  ...
  // Hard guard: a closed sprint never emits an Execute quick-action,
  // regardless of taskCount / tasksDone. The chain's assert-active leaf
  // would reject any execute attempt anyway, but the home pipeline
  // shouldn't dangle a "Continue Work" CTA either. Belt-and-suspenders
  // against stale tasks-in-progress on a manually-closed sprint.
  if (ctx.currentSprintStatus === 'closed') {
    return {
      id: 'execute',
      title: 'Execute',

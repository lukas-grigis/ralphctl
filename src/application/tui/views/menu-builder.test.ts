...
describe('buildBrowseMenu', () => {
  ...
  it('Tickets drills into the ticket submenu', () => {
    const menu = buildBrowseMenu(makeCtx());
    expect(findByName(menu, 'Tickets')?.action).toStrictEqual({ kind: 'subMenu', group: 'ticket' });
  });

  it('Sprints drills into the sprint submenu', () => {
    const menu = buildBrowseMenu(makeCtx());
    expect(findByName(menu, 'Sprints')?.action).toStrictEqual({ kind: 'subMenu', group: 'sprint' });
  });

  it('Projects drills into the project submenu (where Onboard lives)', () => {
    const menu = buildBrowseMenu(makeCtx());
    expect(findByName(menu, 'Projects')?.action).toStrictEqual({ kind: 'subMenu', group: 'project' });
  });

  it('Tasks drills into the task submenu', () => {
    const menu = buildBrowseMenu(makeCtx());
    expect(findByName(menu, 'Tasks')?.action).toStrictEqual({ kind: 'subMenu', group: 'task' });
  });

  it('does not include a top-level "Onboard a repo" entry — onboard lives under Projects submenu only', () => {
    const menu = buildBrowseMenu(makeCtx({ hasProjects: true }));
    expect(findByName(menu, 'Onboard a repo')).toBeUndefined();
  });
});
...
describe('buildSubMenu', () => {
  ...
  it('sprint submenu — Create PR / MR launches the create-pr chain', () => {
    ...
    expect(choice?.action).toStrictEqual({ kind: 'launchChain', flow: 'create-pr' });
  ...
  it('ticket submenu — Refine action launches the refine chain', () => {
    ...
    expect(findByName(menu, 'Refine')?.action).toStrictEqual({ kind: 'launchChain', flow: 'refine' });
  });
  ...
  it('project submenu — includes Onboard repo entry that launches the onboard chain', () => {
    ...
    expect(onboard?.action).toStrictEqual({ kind: 'launchChain', flow: 'onboard' });

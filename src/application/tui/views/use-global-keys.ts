...
import { useCurrentPrompt } from '../../../integration/ui/prompts/hooks.ts';
...
export function useGlobalKeys(sessionManagerOrOptions?: SessionManagerPort | null | Options): void {
  // Accept either the legacy `sessionManager` positional arg or the new
  // options bag for backwards compatibility with existing call sites.
      ...
      if (sessionManager) {
        ...
        if (key.tab && !key.shift) {
          ...
          if (sessions.length > 0) {
            ...
            if (next) {
              ...
              router.push({ id: 'execute', props: { sessionId: next.id } });
            }
          }
          return;
        }
        if (key.tab && key.shift) {
          ...
          if (sessions.length > 0) {
            ...
            if (prev) {
              ...
              router.push({ id: 'execute', props: { sessionId: prev.id } });
            }
          }
          return;
        }
        ...
        if (key.ctrl && /^[1-9]$/.test(input)) {
          ...
          if (target) {
            ...
            router.push({ id: 'execute', props: { sessionId: target.id } });
          }
          return;
        }
      }

...
import { PromptHost } from '../../../integration/ui/prompts/prompt-host.tsx';
...
import { TicketAssignReposView } from './crud/ticket-assign-repos-view.tsx';
...
> = {
  ...
  'ticket-assign-repos': {
    ...
    render: (props) => {
      const ticketId = typeof props['ticketId'] === 'string' ? props['ticketId'] : undefined;
      return <TicketAssignReposView ticketId={ticketId} />;
    },
  },
...
export function ViewRouter({ initialStack, sessionManager, signalBus = null }: Props): React.JSX.Element {
  ...
  // Active session label for the status bar sessions indicator
  const [activeSession, setActiveSession] = useState<SessionDescriptor | null>(() => sessionManager?.active ?? null);
  useEffect(() => {
    if (!sessionManager) return;
    const unsub = sessionManager.subscribe(() => {
      setActiveSession(sessionManager.active ?? null);
    });
    return unsub;
  }, [sessionManager]);

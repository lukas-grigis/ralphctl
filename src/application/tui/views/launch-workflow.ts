...
import type { SessionId, SessionManagerPort } from '../../runtime/session-manager-port.ts';
 ...
 * ticket's first affected repo, else the first registered project's first
 * repo, else `process.cwd()`.
 */
  ...
  try {
    ...
    if (config.ok && config.value.currentSprint) {
      ...
      if (idResult.ok) {
        ...
        if (sprintResult.ok) {
          for (const ticket of sprintResult.value.tickets) {
            if (ticket.affectedRepositories && ticket.affectedRepositories.length > 0) {
              const first = ticket.affectedRepositories[0];
...
async function buildRefineInputs(deps: SharedDeps, router: RouterApi): Promise<FlowInputs | null> {
  ...
  return { flow: 'refine', sprintId, cwd, pendingTickets };
}

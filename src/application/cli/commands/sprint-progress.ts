...
} from '@src/business/usecases/sprint/show-progress.ts';
import { Result } from '@src/domain/result.ts';
import { IsoTimestamp } from '@src/domain/values/iso-timestamp.ts';
import type { SprintId } from '@src/domain/values/sprint-id.ts';
import { SprintId as SprintIdNs } from '@src/domain/values/sprint-id.ts';
...
async function runSprintProgress(deps: SharedDeps, id: string | undefined, opts: ProgressOptions): Promise<ExitCode> {
  ...
  return runCommand({
    ...
    body: async () => {
      ...
      if (!idR.ok) {
        ...
        const { ValidationError } = await import('@src/domain/values/validation-error.ts');
        return Result.error(
          new ValidationError({
            field: 'sprint.id',
            value: id,

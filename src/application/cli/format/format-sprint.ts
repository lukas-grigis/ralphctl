...
import type { Sprint, SprintStatus } from '@src/domain/entities/sprint.ts';
...
export function formatSprintLine(sprint: Sprint): string {
  ...
  const project = c.dim(`[${String(sprint.projectName)}]`);
  const ticketCount = c.dim(`${String(sprint.tickets.length)} ticket(s)`);
  return `${status.padEnd(16)} ${id}  ${name}  ${project}  ${ticketCount}`;
}
...
export function formatSprintCard(sprint: Sprint): string {
  ...
  lines.push(`  ${c.dim('project ')} ${String(sprint.projectName)}`);
  ...
  if (sprint.affectedRepositories.length > 0) {
    lines.push(`  ${c.dim('repos   ')} ${sprint.affectedRepositories.map(String).join(', ')}`);
  }

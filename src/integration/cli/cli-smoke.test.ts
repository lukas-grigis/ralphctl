...
describe('CLI Smoke Tests', { timeout: 5000 }, () => {
  ...
  it('updates task status', async () => {
    ...
    const { activateSprint, getSprint } = await import('@src/integration/persistence/sprint.ts');
    const { getCurrentSprint } = await import('@src/integration/persistence/config.ts');
  ...
  it('sets and shows AI provider config', async () => {
    ...
    expect(setInvalid.stdout).toContain('Invalid value for aiProvider');
  });
});

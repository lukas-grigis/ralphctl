 ...
 */
export async function checkConfigSchemaValidation(): Promise<CheckResult> {
  const config = await getConfig();
  const entries = getAllConfigSchemaEntries();
  const invalid: string[] = [];

  for (const entry of entries) {
    const value = config[entry.key as keyof Config];
    // Skip null/undefined — those are handled by individual checks (e.g. checkEvaluationConfig)
    if (value == null) continue;
    if (!entry.validation(value)) {
      invalid.push(`${entry.key}: ${JSON.stringify(value)} (${entry.description})`);
    }
  }

  if (invalid.length === 0) {
    return { name: 'Config schema validation', status: 'pass', detail: 'all values valid' };
  }

  return {
    name: 'Config schema validation',
    status: 'warn',
    detail: `invalid values: ${invalid.join('; ')}`,
  };
}

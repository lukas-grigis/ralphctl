...
import { formatTicketId, getPendingRequirements } from '@src/integration/persistence/ticket.ts';
import {
  executeTaskLoop,
  executeTaskLoopParallel,
  type ExecutionSummary,
  type ExecutorOptions,
} from '@src/integration/ai/executor.ts';
import {
  type CheckResults,
  type CheckStatus,
...
export type { ExecutorOptions, ExecutionSummary } from '@src/integration/ai/executor.ts';
...
export async function promptBranchStrategy(sprintId: string): Promise<string | null> {
  ...
  const strategy = await getPrompt().select({
    message: 'How should this sprint manage branches?',
    choices: [
      {
        label: `Create sprint branch: ${autoBranch} (Recommended)`,
        value: 'auto',
      },
      {
        label: 'Keep current branch (no branch management)',
        value: 'keep',
      },
      {
  ...
  // Custom branch name
  const customName = await getPrompt().input({
...
export async function runSprint(
  ...
  if (sprint.status === 'draft' && !options.force) {
    ...
    if (unrefinedTickets.length > 0) {
      ...
      const shouldContinue = await getPrompt().confirm({

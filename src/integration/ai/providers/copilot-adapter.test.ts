...
describe('copilotAdapter', () => {
  ...
  describe('buildInteractiveArgs', () => {
    it('uses -i PROMPT (not --)', () => {
      ...
      expect(args).toStrictEqual(['--allow-all-tools', '-i', 'test prompt']);
    });

    it('places extras between baseArgs and -i', () => {
      const args = copilotAdapter.buildInteractiveArgs('p', ['--model', 'gpt-4']);
      expect(args).toStrictEqual(['--allow-all-tools', '--model', 'gpt-4', '-i', 'p']);
    });
  });
  ...
  describe('parseJsonOutput', () => {
    it('parses single-line JSON', () => {
      ...
      expect(copilotAdapter.parseJsonOutput(out)).toStrictEqual({
        result: 'done',
        sessionId: 's1',
        model: null,
        numTurns: null,
    ...
    it('handles empty input', () => {
      expect(copilotAdapter.parseJsonOutput('')).toStrictEqual({
        result: '',
        sessionId: null,
        model: null,
        numTurns: null,
  ...
  describe('buildResumeArgs', () => {
    it('uses --resume=<id> (single arg)', () => {
      expect(copilotAdapter.buildResumeArgs('abc123')).toStrictEqual(['--resume=abc123']);
    });
  ...
  describe('getSpawnEnv', () => {
    it('returns an empty record', () => {
      expect(copilotAdapter.getSpawnEnv()).toStrictEqual({});
    });
  });
});

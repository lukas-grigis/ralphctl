...
describe('claudeAdapter', () => {
  describe('metadata', () => {
    ...
    it('baseArgs does NOT embed --permission-mode (split per mode)', () => {
      // Interactive uses acceptEdits (safer, prompts for Bash/Web);
      // headless uses bypassPermissions (acceptEdits would deadlock
      // claude -p on a Bash prompt). Each builder adds its own flag.
  ...
  describe('buildInteractiveArgs', () => {
    it('uses acceptEdits + adds prompt after --', () => {
      ...
      expect(args).toStrictEqual(['--permission-mode', 'acceptEdits', '--effort', 'xhigh', '--', 'test prompt']);
    });

    it('includes extra args before the prompt', () => {
      const args = claudeAdapter.buildInteractiveArgs('p', ['--verbose']);
      expect(args).toStrictEqual(['--permission-mode', 'acceptEdits', '--effort', 'xhigh', '--verbose', '--', 'p']);
    });
  ...
  describe('parseJsonOutput', () => {
    ...
    it('handles empty input', () => {
      expect(claudeAdapter.parseJsonOutput('')).toStrictEqual({
        result: '',
        sessionId: null,
        model: null,
        numTurns: null,
  ...
  describe('buildResumeArgs', () => {
    it('returns space-separated --resume + sessionId', () => {
      expect(claudeAdapter.buildResumeArgs('abc123')).toStrictEqual(['--resume', 'abc123']);
    });

    it('accepts hyphens and underscores after the first character', () => {
      expect(claudeAdapter.buildResumeArgs('s_1-2-3')).toStrictEqual(['--resume', 's_1-2-3']);
    });
  ...
  describe('getSpawnEnv', () => {
    it('exposes CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1', () => {
      expect(claudeAdapter.getSpawnEnv()).toStrictEqual({

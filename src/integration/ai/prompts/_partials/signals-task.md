...
- `<change>text</change>` — A concrete change you made during this task. Granular ("added X", "renamed Y to Z", "deleted Z"). The harness renders these under `#### Changes` inside this task's per-task section of `progress.md` on the next snapshot regeneration.
- `<learning>text</learning>` — Non-obvious project knowledge worth carrying across tasks (a hidden constraint, an undocumented convention, a gotcha you hit and resolved). The harness renders these under `#### Learnings` inside this task's per-task section of `progress.md`. They survive to subsequent sessions when the next AI reads `progress.md` at the start of Phase 1. Use sparingly; only the kind of insight you'd want a fresh agent to read first.
- `<note>text</note>` — Incidental observations future tasks should be aware of (patterns, gotchas). The harness renders these under `#### Notes` inside this task's per-task section of `progress.md`.
- `` `<decision>` `` ... `` `</decision>` `` — Wrap an architectural or design choice with rationale. Authoring rules (one sentence, sparingly, no embedded headings or fences) live in the "Recording architectural decisions" section above; the tag mechanics live here. The harness renders these under `## Decisions` in `progress.md` (capped at 500 chars per body). Emit ONE decision per pair of tags.

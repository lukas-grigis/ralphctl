...
function makeSprint(tickets: readonly Ticket[] = []): Sprint {
  const r0 = Sprint.create({ name: 'Sprint Alpha', slug: slug('sprint-alpha'), now: T0 });
...
describe('TextPromptBuilderAdapter — refine', () => {
  it('loads the ticket-refine template and substitutes ticket marker', async () => {
    ...
    if (!r.ok) return;
    expect(loader.calls).toEqual(['ticket-refine']);
...
describe('TextPromptBuilderAdapter — plan', () => {
  it('renders an empty PROJECT_TOOLING slot when no repos are detected', async () => {
    // Sprint with no `affectedRepositories` → detectProjectTooling([])
    // → empty rendered string. The placeholder is consumed but produces
    // nothing visible in the prompt.
    const loader = new StubTemplateLoader({
      'plan-common': '<<{{PROJECT_TOOLING}}>>',
    });
    const adapter = new TextPromptBuilderAdapter(loader);
...
describe('TextPromptBuilderAdapter — onboard', () => {
  it('substitutes every onboarding placeholder', async () => {
    ...
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    expect(loader.calls).toEqual(['repo-onboard']);

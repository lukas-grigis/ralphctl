...
vi.mock('@src/integration/ai/task-context.ts', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@src/integration/ai/task-context.ts')>();
  return {
    ...actual,
...
vi.mock('@src/integration/ai/lifecycle.ts', () => ({
...
describe('runCheckScripts', () => {
  ...
  beforeEach(async () => {
    ...
    const { runLifecycleHook } = await import('@src/integration/ai/lifecycle.ts');
    vi.mocked(runLifecycleHook).mockReturnValue({ passed: true, output: 'ok' });

    const sprintStore = await import('@src/integration/persistence/sprint.ts');
    ...
    const taskContext = await import('@src/integration/ai/task-context.ts');
  ...
  it('records timestamp on successful check run', async () => {
    ...
    const taskContext = await import('@src/integration/ai/task-context.ts');
    const { saveSprint } = await import('@src/integration/persistence/sprint.ts');
...
vi.mock('@src/integration/external/git.ts', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@src/integration/external/git.ts')>();
  return {
    ...actual,
...
describe('ensureSprintBranches', () => {
  ...
  beforeEach(async () => {
    ...
    const taskStore = await import('@src/integration/persistence/task.ts');
    vi.mocked(taskStore.getTasks).mockResolvedValue([]);

    const sprintStore = await import('@src/integration/persistence/sprint.ts');
    vi.mocked(sprintStore.saveSprint).mockResolvedValue(undefined);

    const git = await import('@src/integration/external/git.ts');

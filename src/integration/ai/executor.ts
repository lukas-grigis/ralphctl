...
import { getHeadSha, hasUncommittedChanges } from '@src/integration/external/git.ts';
import { writeEvaluation } from '@src/integration/persistence/evaluation.ts';
import { highlight, info, muted, success, warning } from '@src/integration/ui/theme/theme.ts';
...
import { getProgress, logProgress, summarizeProgressForContext } from '@src/integration/persistence/progress.ts';
import { getProgressFilePath, getSprintDir } from '@src/integration/persistence/paths.ts';
import { buildEvaluationResumePrompt, buildTaskExecutionPrompt } from '@src/integration/ai/prompts/loader.ts';
import type { Project, Task } from '@src/domain/models.ts';
import { createSpinner, formatTaskStatus } from '@src/integration/ui/theme/ui.ts';
import { type ExecutionResult, parseExecutionResult } from '@src/integration/ai/parser.ts';
import type { SpawnResult } from '@src/integration/ai/providers/types.ts';
import { type ProviderAdapter } from '@src/integration/ai/providers/types.ts';
import { spawnInteractive, spawnWithRetry } from '@src/integration/ai/session.ts';
import { SpawnError } from '@src/domain/errors.ts';
import { RateLimitCoordinator } from '@src/integration/ai/rate-limiter.ts';
import { EXIT_ALL_BLOCKED, EXIT_ERROR, EXIT_NO_TASKS, EXIT_SUCCESS } from '@src/integration/utils/exit-codes.ts';
...
} from '@src/integration/ai/task-context.ts';
import { runLifecycleHook } from '@src/integration/ai/lifecycle.ts';
import { getActiveProvider } from '@src/integration/ai/providers/registry.ts';
import { verifySprintBranch } from '@src/integration/ai/runner.ts';
import { getEvaluationIterations } from '@src/integration/persistence/config.ts';
import { type EvaluationResult, runEvaluation } from '@src/integration/ai/evaluator.ts';
...
export interface ExecutorOptions {
  ...
  maxTurns?: number;
  /** Skip the sprint feedback loop after all tasks complete */
  noFeedback?: boolean;
}
  ...
  while (completedCount < targetCount) {
    ...
    if (options.step && completedCount < targetCount) {
      ...
      if (remaining.length > 0) {
        ...
        const continueLoop = await getPrompt().confirm({

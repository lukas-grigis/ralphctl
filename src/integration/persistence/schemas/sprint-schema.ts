...
function fromTicket(ticket: Ticket): TicketJson {
  return {
    ...
    ...(ticket.affectedRepositories !== undefined ? { affectedRepositories: [...ticket.affectedRepositories] } : {}),
...
function toTicket(parsed: TicketJson): Result<Ticket, StorageError> {
  ...
  if (parsed.affectedRepositories !== undefined) {
    t = t.assignRepositories(parsed.affectedRepositories.map((p) => AbsolutePath.trustString(p)));
  }

...
describe('task-schema', () => {
  it('round-trips a freshly-created (todo) task', () => {
    ...
    expect(back.value.steps).toEqual(['analyze', 'implement', 'test']);
  });
  ...
  it('preserves blockedBy and ticketId', () => {
    ...
    expect(back.value.blockedBy).toEqual([dep]);
  });
  ...
  it('preserves extraDimensions when present', () => {
    ...
    expect(back.ok).toBe(true);
    if (!back.ok) return;
    expect(back.value.extraDimensions).toEqual(['Performance', 'Security']);
  });

...
describe('FileTaskRepository', () => {
  ...
  it('saveAll then findBySprintId round-trip the task list', async () => {
    ...
    expect(r.value.map((t) => t.id).sort()).toStrictEqual([t1.id, t2.id].sort());
  });
  ...
  it('preserves order across multiple updates', async () => {
    ...
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    expect(r.value.map((t) => t.id)).toStrictEqual([t1.id, t2.id]);
  ...
  it('round-trips a task with all optional fields populated', async () => {
    ...
    expect(loaded.verificationCriteria).toStrictEqual(['check-1', 'check-2']);
    expect(loaded.extraDimensions).toStrictEqual(['Performance', 'Maintainability']);

...
describe('PullRequestRunner.create', () => {
  it('runs `gh pr create` and returns the URL on success', () => {
    ...
    if (!r.ok) return;
    expect(r.value).toEqual({ url: 'https://github.com/acme/widgets/pull/42', platform: 'github' });
    expect(calls[0]?.command).toBe('gh');
    expect(calls[0]?.args.slice(0, 2)).toEqual(['pr', 'create']);
  ...
  it('runs `glab mr create` for gitlab remotes', () => {
    ...
    expect(calls[0]?.args.slice(0, 2)).toEqual(['mr', 'create']);

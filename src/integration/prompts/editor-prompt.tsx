...
function splitLines(text: string): string[] {
  // Preserve a trailing empty line if the text ends in \n.
  return text.split('\n');
}
...
export function EditorPrompt({ options, onSubmit, onCancel }: EditorPromptProps): React.JSX.Element {
  ...
  useInput((input, key) => {
    ...
    if (key.backspace || key.delete) {
      // Ink reports delete and backspace somewhat inconsistently — treat both
      // as "remove char behind cursor" for muscle memory.
      setLines((prev) => {
        const next = [...prev];
    ...
    if (input && !key.ctrl && !key.meta) {
      // Handle paste (multi-char chunk) and plain typing the same way —
      // Ink's useInput hands us the whole chunk in one call.
      const chunk = input;
      setLines((prev) => {
        const next = [...prev];
        const line = next[cursor.row] ?? '';
        ...
        // Replace current line with first part; insert the rest as new lines.
        next.splice(cursor.row, 1, ...parts);
        // Cursor lands at end of the inserted text (before `after`).
  ...
  // Render: show all lines, indicating the cursor position with a visible
  // inverse-video block. Empty lines show as a single cursor when active.
  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor="gray" paddingX={1}>
      ...
      <Box marginTop={1}>
        <Text dimColor>
          Ctrl+D submit · Esc cancel · Enter newline
        </Text>
      </Box>
    </Box>
  );

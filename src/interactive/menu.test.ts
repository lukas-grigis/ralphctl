...
import { buildMainMenu, buildSubMenu, type MenuContext } from './menu.ts';

/** Helper to extract actionable choices (not separators) from menu items */
function choices(items: ({ value?: string } | InstanceType<typeof Separator>)[]) {
  return items.filter((i): i is { name: string; value: string } => !(i instanceof Separator) && 'value' in i);
}

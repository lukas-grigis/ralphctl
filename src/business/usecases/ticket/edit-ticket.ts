 ...
 * input. Lifecycle state (`requirementStatus`, `requirements`,
 * `affectedRepositories`) is reset to defaults by the factory; that's
 * acceptable here because edit operates on draft tickets only — the
 * sprint's `replaceTicket` enforces draft status, and an `approved` ticket
 * shouldn't be edited via this path. (Approval / repo assignment have
 * dedicated use cases.)
 */
export class EditTicketUseCase {
  ...
  async execute(input: EditTicketInput): Promise<Result<Sprint, DomainError>> {
    ...
    const merged = Ticket.create({
      ...
      projectName: input.partial.projectName ?? current.projectName,
    });

...
describe('parseProgressTimeline', () => {
  it('returns an empty list for empty input', () => {
    expect(parseProgressTimeline('')).toStrictEqual([]);
  });
...
describe('ShowProgressUseCase', () => {
  it('returns NotFoundError for missing sprint', async () => {
    ...
    const sprintId = unwrap(
      Sprint.create({
        name: 'X',
        slug: unwrap(Slug.parse('x')),
        now: NOW,
        projectName: unwrap(ProjectName.parse('demo')),
      })
    ).id;
  ...
  it('returns an empty report for a sprint with no tasks', async () => {
    ...
    if (!result.ok) return;
    expect(result.value.tasks).toStrictEqual([]);
    expect(result.value.timeline).toStrictEqual([]);
    expect(result.value.blockers).toStrictEqual([]);
    expect(result.value.staleTasks).toStrictEqual([]);
    expect(result.value.dependencyCycle).toBeNull();
    expect(result.value.branchInconsistency).toStrictEqual([]);
  ...
  it('does not flag in_progress tasks with a recent signal', async () => {
    ...
    const result = unwrap(await uc.execute({ sprintId: sprint.id, now: NOW }));
    expect(result.staleTasks).toStrictEqual([]);
  });
  ...
  it('skips branch checks when sprint has no branch', async () => {
    ...
    const result = unwrap(await uc.execute({ sprintId: sprint.id, now: NOW }));
    expect(result.branchInconsistency).toStrictEqual([]);
  });

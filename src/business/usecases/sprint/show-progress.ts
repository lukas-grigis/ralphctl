...
import { topologicalReorder } from '../../../kernel/algorithms/dependency-reorder.ts';
...
function detectBranchInconsistency(
  ...
  for (const ticket of sprint.tickets) {
    if (ticket.affectedRepositories === undefined) continue;
    for (const path of ticket.affectedRepositories) {
      repoPaths.add(path);
    }
  }
  // Fall back to every repo across registered projects when no ticket has
  // been assigned repos yet — gives the user actionable feedback even
  // before planning.
  if (repoPaths.size === 0) {

...
describe('Retry', () => {
  ...
  it('onTrace fires once per attempt with the renamed step name', async () => {
    ...
    expect(seen.map((e) => e.stepName)).toStrictEqual(['do-it#attempt-1', 'do-it#attempt-2', 'do-it#attempt-3']);
    expect(seen.map((e) => e.status)).toStrictEqual(['failed', 'failed', 'completed']);
  });

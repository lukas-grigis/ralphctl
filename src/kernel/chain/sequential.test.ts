...
describe('Sequential', () => {
  it('runs children in order and threads ctx through each', async () => {
    ...
    if (!result.ok) return;
    expect(result.value.ctx.path).toStrictEqual(['a', 'b', 'c']);
    expect(result.value.trace.map((t) => t.stepName)).toStrictEqual(['first', 'second', 'third']);
  ...
  it('first failure short-circuits and remaining children appear as skipped', async () => {
    ...
    expect(trace.map((t) => t.stepName)).toStrictEqual(['a', 'boom', 'c', 'd']);
  ...
  it('abort mid-flight: current step aborts, remaining are skipped', async () => {
    ...
    if (result.ok) return;
    const trace = result.error.trace;
    expect(trace.map((t) => t.stepName)).toStrictEqual(['quick', 'slow', 'after']);
    expect(trace[0]?.status).toBe('completed');
  ...
  it('empty children list completes immediately with empty trace', async () => {
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.value.ctx.path).toStrictEqual(['untouched']);
  ...
  it('onTrace fires progressively per child in execution order', async () => {
    ...
    expect(seen.map((e) => e.stepName)).toStrictEqual(['first', 'second', 'third']);
  ...
  it('onTrace fires for completed children then synthetic skipped entries on failure', async () => {
    ...
    expect(seen.map((e) => e.stepName)).toStrictEqual(['a', 'boom', 'c']);
    expect(seen.map((e) => e.status)).toStrictEqual(['completed', 'failed', 'skipped']);
  });

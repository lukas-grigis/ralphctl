...
describe('OnError', () => {
  ...
  it('runs fallback on child failure when catchIf matches', async () => {
    ...
    expect(names).toStrictEqual(['child', 'fallback']);
  ...
  it('propagates the original error when catchIf returns false', async () => {
    ...
    expect(result.error.trace.map((t) => t.stepName)).toStrictEqual(['child']);
    expect(fallbackSpy).not.toHaveBeenCalled();
  });
  ...
  it('onTrace fires for both child failure and fallback success', async () => {
    ...
    expect(seen.map((e) => e.stepName)).toStrictEqual(['child', 'fallback']);
    expect(seen.map((e) => e.status)).toStrictEqual(['failed', 'completed']);
  });
  ...
  it('propagates fallback failure (no recursive catch)', async () => {
    ...
    expect(trace.map((t) => t.stepName)).toStrictEqual(['child', 'fallback']);
    expect(trace.map((t) => t.status)).toStrictEqual(['failed', 'failed']);
  });
});

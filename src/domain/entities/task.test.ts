...
describe('Task.create', () => {
  it('builds a todo task with sane defaults', () => {
    ...
    expect(t.blockedBy).toStrictEqual([]);
    expect(t.id).toMatch(/^[0-9a-f]{8}$/);
    expect(t.steps).toStrictEqual(['step 1']);
    expect(t.verificationCriteria).toStrictEqual(['runs']);
  ...
  it('preserves blockedBy and ticketId references', () => {
    ...
    expect(t.blockedBy).toStrictEqual(['aaaaaaaa', 'bbbbbbbb']);
  });

  it('preserves extraDimensions', () => {
    const t = freshTask({ extraDimensions: ['Performance', 'Security'] });
    expect(t.extraDimensions).toStrictEqual(['Performance', 'Security']);
  });
});
...
describe('Task status transitions', () => {
  ...
  it('in_progress → in_progress is idempotent (resume support)', () => {
    // The harness re-runs `mark-in-progress` when resuming an interrupted
    // sprint. Returning the same instance unchanged is the only safe
    // behaviour — rejecting blew up the per-task chain and stranded the
    // runner.
    const t = freshTask();
    ...
    if (r2.ok) {
      expect(r2.value.status).toBe('in_progress');
      // No allocation: the same instance is returned to skip a redundant
      // persist on the resume path.
  ...
  it('refuses blocked → in_progress (caller must unblock first)', () => {
    const blockedR = freshTask().markBlocked('reason');
    if (!blockedR.ok) throw new Error('precondition failed');
    const r = blockedR.value.markInProgress();
    expect(r.ok).toBe(false);
    if (!r.ok) {
      expect(r.error.code).toBe('invalid-state');
      expect(r.error.attemptedAction).toBe('mark-in-progress');
    }
  });
...
describe('Task.setBlockedBy', () => {
  it('overwrites the dependency list', () => {
    ...
    expect(t2.blockedBy).toStrictEqual(['bbbbbbbb', 'cccccccc']);
    // immutability
    expect(t1.blockedBy).toStrictEqual(['aaaaaaaa']);
  });

  it('clears with empty array', () => {
    const t1 = freshTask({ blockedBy: [taskId('aaaaaaaa')] });
    const t2 = t1.setBlockedBy([]);
    expect(t2.blockedBy).toStrictEqual([]);
  });
});
...
describe('Task.update', () => {
  ...
  it('updates description, steps, verificationCriteria, blockedBy, projectPath', () => {
    ...
    expect(r.value.steps).toStrictEqual(['s1', 's2']);
    expect(r.value.verificationCriteria).toStrictEqual(['vc1']);
    expect(r.value.blockedBy).toStrictEqual(['aaaaaaaa']);

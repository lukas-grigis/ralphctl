 ...
 *
 * Project scope is sprint-level (every ticket inherits the sprint's
 * `projectName`); affected repositories are sprint-level too (set during
 * `sprint plan`). The ticket itself only carries the user-authored intent
 * (title / description / link) and its refinement state.
 *

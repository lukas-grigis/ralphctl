...
describe('readiness fan-out across unique providers', () => {
  ...
  it('multi-repo + pre-selected repositoryId → pick-repository auto-resolves the picked repo without a second prompt', async () => {
    // Two repos on the project. The pre-launch picker already chose repo-b; the launcher threads
    // its id as `opts.repositoryId` + `initialCtx.repositoryId`. `pickRepositoryLeaf` must take the
    // preselect branch (no `askChoice`) and resolve repo-b — NOT the project's first repo (repo-a).
    const repoBPath = join(tmpDir, 'repo-b');
    await fs.mkdir(repoBPath, { recursive: true });
    const repoA = makeRepository({
      id: repositoryId('01900000-0000-7000-8000-0000000000a1'),
      path: repoPath,
      name: 'repo-a',
    });
    const repoB = makeRepository({
      id: repositoryId('01900000-0000-7000-8000-0000000000b2'),
      path: repoBPath,
      name: 'repo-b',
    });
    const project = makeProject({ repositories: [repoA, repoB] });

    const eventBus = createInMemoryEventBus();
    const writer = recordingWriteFile();
    const provider = createFakeAiProvider({
      signals: { readiness: [agentsMdProposal('# repo-b — generated by AI\n')] },
    });

    // Count repository-choice prompts: on the preselect path this must stay 0. An error return
    // makes a regression loud — any `askChoice` call would also fail the chain.
    let repoChoiceCalls = 0;
    const interactive: InteractivePrompt = {
      ...scriptedInteractive({ confirms: [true] }),
      async askChoice<T>(): Promise<Result<T, DomainError>> {
        repoChoiceCalls += 1;
        return Result.error(
          new ValidationError({
            field: 'fake',
            value: null,
            message: 'pick-repository must not prompt on the preselect path',
          })
        ) as Result<T, DomainError>;
      },
    };
    ...
    const flow = createReadinessFlow(
      ...
      { projectId: project.id, repositoryId: repoB.id, cwd: FAKE_CWD, ai }
    );

    const runner = createRunner({
      id: 'r-fanout-preselect',
      element: flow,
      initialCtx: { projectId: project.id, repositoryId: repoB.id, tools: [], entries: {} },
    });
    await runner.start();

    expect(runner.status).toBe('completed');
    // No interactive repository choice fired — the preselected repo resolved silently.
    expect(repoChoiceCalls).toBe(0);
    // The chain resolved the PICKED repo (repo-b), not the project's first repo (repo-a).
    expect(runner.ctx.repository?.id).toBe(repoB.id);
    // The native context file was written against repo-b, never repo-a.
    const targets = filterTargetWrites(writer.writes).map((w) => w.path);
    expect(targets).toEqual([join(repoBPath, 'CLAUDE.md')]);
    expect(targets.some((p) => p.startsWith(repoPath))).toBe(false);
  });
});

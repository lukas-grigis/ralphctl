...
describe('SettingsView', () => {
  ...
  describe('provider availability gate', () => {
    ...
    it('surfaces the install command in the picker footer when a provider is unavailable', async () => {
      ...
      // Footer names the PATH binary (copilot / codex / grok), not the provider id, plus the
      // OS-preferred command (brew on macOS, winget on Windows, npm on Linux). Assert on the
      // OS-invariant prefix plus an alternation over the per-OS Copilot CLI install fragments —
      // proving the install COMMAND rendered (not just the picker label, which already contains
      // "copilot").
      expect(frame).toMatch(/install copilot: \S/);
      expect(frame).toMatch(/copilot-cli|@github\/copilot|GitHub\.Copilot/);
      expect(frame).toMatch(/install codex: \S/);

...
import {
  ...
  resolveWritableLearningsLedgerPath,
} from '@src/application/flows/_shared/memory/ledger-path.ts';

describe('resolveWritableLearningsLedgerPath (tolerant write-side resolver — never splits)', () => {
  const PROJECT_ID = 'proj-1';

  it('builds the slugged <id>--<slug>/ path when NO memory dir exists yet (brand-new project)', async () => {
    const tmp = await makeTmpRoot();
    try {
      const memoryRoot = absolutePath(join(String(tmp.root), 'memory'));
      const result = await resolveWritableLearningsLedgerPath(memoryRoot, PROJECT_ID, slug('demo'));
      expect(result.ok).toBe(true);
      if (!result.ok) return;
      expect(String(result.value)).toBe(
        join(String(memoryRoot), buildSluggedName(PROJECT_ID, 'demo'), LEARNINGS_LEDGER_FILE)
      );
    } finally {
      await tmp.cleanup();
    }
  ...
  it('writes into the EXISTING legacy bare <id>/ dir — no second dir, no stranding', async () => {
    const tmp = await makeTmpRoot();
    try {
      const memoryRoot = absolutePath(join(String(tmp.root), 'memory'));
      ...
      expect(result.ok).toBe(true);
      if (!result.ok) return;
      // Critical: keep appending to the legacy bare dir, NOT a freshly-built slugged sibling.
  ...
  it('prefers the slugged <id>--<slug>/ dir when both forms exist', async () => {
    const tmp = await makeTmpRoot();
    try {
      const memoryRoot = absolutePath(join(String(tmp.root), 'memory'));
